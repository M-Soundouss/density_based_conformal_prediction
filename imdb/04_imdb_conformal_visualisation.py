...
subset_idx_0 = np.random.choice(range(len(density_values[0][3])), size=1000)
subset_idx_1 = np.random.choice(range(len(density_values[1][3])), size=1000)

plt.scatter(density_values[0][3][subset_idx_0], density_values[0][4][subset_idx_0], c="r", s=0.3,
            alpha=0.4, label="Class 0 : Negative")
plt.scatter(density_values[1][3][subset_idx_1], density_values[1][4][subset_idx_1], c="b", s=0.3,
            alpha=0.4, label="Class 1 : Positive")
plt.legend()
plt.xlim(-3, 3)
plt.ylim(-3, 3)
...
#plt.axis('off')
title = 'IMDb : ' + r'$\epsilon$' + ' = 0.05'
plt.title(title)

plt.savefig('imdb_density_eng.eps', format='eps')
plt.close()
